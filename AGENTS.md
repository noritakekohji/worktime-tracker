# AGENTS.md — worktime-tracker AI コーディング指針

このリポジトリで Codex (および他の AI アシスタント) が作業するときに **必ず参照する** 規約集。  
...
5. git commit (Co-Authored-By: Codex ... タグ付き)
6. git push
```
...
| 保存した JSON の配列がオブジェクト/文字列になる | `_ToPSObjectDeep` 等が `return $arr` で 1 要素配列を unwrap。`roles`=1個 → `"roles":"member"`、`wbs_items`/`processes`/`task_groups`/`tasks` も 1 件だとオブジェクトに潰れる | 配列を返す関数はすべて `return ,$arr`。読込側は `@()` で囲むので**ツール内では動いてしまい**、壊れるのはファイルの中身だけ。生 JSON を検証するテストで押さえる (`tests/lib/MasterSchema.Tests.ps1`) |
...
| `scripts/import-excel-workentries.ps1` | Excel の作業実績を月次 JSON に変換して取り込む。出力先は `-OutputRoot` 配下のみで `local_store` / Gitlab には書かない。**Microsoft Excel が必要** (Excel COM) |
