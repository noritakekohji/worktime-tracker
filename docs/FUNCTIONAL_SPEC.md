...
**カスケード絞り込み (WBS 連動)**: 選択プロジェクトに `wbs_items` が登録されている場合、Tracker の カスケード (工程/タスクグループ/タスク) は WBS で使用宣言されている項目のみ表示。`wbs_items` 未登録のプロジェクトは task_pattern 全項目を表示 (従来動作)。WBS でグループレベル登録 (`task_code='-'`) がある場合、タスクプルダウンに `(タスクグループ全体)` が選択肢に追加。
...
| 🏷 カテゴリ別 | カテゴリ単位の合計・件数 |
| 🖥 システム別 | `projects.target_system` 解決で対象システム別の合計・件数 |
| 🏢 会社別 | `members.company` 解決で会社別の合計・件数 (グループ会社/協力会社の負荷可視化) |
| 📈 分析 | 詳細クロス集計 (任意軸) |
